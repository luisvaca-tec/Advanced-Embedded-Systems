...
* The LED program from previous example is used to turn
* on the tri-color LEDs according to the key received.
* * By default in SystemInit(), FLL clock output is 41.94 MHz. * Setting BDH=0, BDL=0x17, and OSR=0x0F yields 115200 Baud.
*/
...
void LED_set(int value)
{
/* change LEDs according to bit 2-0 of the value */
if (value & 1) /* use bit 0 of count to control red LED */
PTB->PCOR = 0x40000; /* turn on red LED */
else
PTB->PSOR = 0x40000; /* turn off red LED */
if (value & 2) /* use bit 1 of count to control green LED */
PTB->PCOR = 0x80000; /* turn on green LED */
else
PTB->PSOR = 0x80000; /* turn off green LED */
if (value & 4) /* use bit 2 of count to control blue LED */
